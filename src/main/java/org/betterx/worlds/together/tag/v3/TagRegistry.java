...
public class TagRegistry<T> {
            ...
            synchronized (this) {
                ...
                for (ResourceKey<Biome> element : elements) {
                    ...
                    for (TagEntry tagEntry : set) {
                        if (!tagEntry.isTag() && tagEntry.getId().equals(id)) {
                            id = null;
                            break;
                        }
                    }
    ...
    public void forEachTag(
        ...
        tags.forEach((tag, set) -> {
            ...
            set.forEach(e -> {
                ResourceLocation id = e.getId();
                if (allow == null || allow.test(tag, id)) {
                    if (e.isTag()) {
    ...
    public void forEachEntry(
        ...
        tags.forEach((tag, set) -> {
            ...
            set.forEach(e -> {
                ...
                if (allow == null || allow.test(tag, id)) {
                    if (e.isTag()) {
                        tags.add(new Pair<>(TagKey.create(registryKey, id), e));
                    } else {
                        locations.add(new Pair<>(id, e));
                    }
                }
            });
