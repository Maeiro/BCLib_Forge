...
public final class BCLWoodTypeWrapper {
            ...
            for (Constructor<?> ctor : WoodType.class.getDeclaredConstructors()) {
                ...
                for (int i = 0; i < params.length; i++) {
                    if (params[i] == String.class) {
                        // Use namespaced id so sign textures resolve to the correct mod namespace.
                        args[i] = id.toString();
    ...
    private static WoodType findExistingWoodType(ResourceLocation id) {
        try {
            ...
            for (Field field : WoodType.class.getDeclaredFields()) {
                ...
                WoodType found = findWoodTypeInSet(values, id);
                if (found != null) return found;
            }
    ...
    private static WoodType findWoodTypeInSet(Iterable<?> values, ResourceLocation id) {
        String path = id.getPath();
        String namespaced = id.toString();
        for (Object entry : values) {
            if (entry instanceof WoodType type
                    && (type.name().equals(path) || type.name().equals(namespaced))) {
                return type;
            }
        }
        return null;
