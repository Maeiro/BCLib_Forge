...
public abstract class ComplexMaterial {
    ...
    public ComplexMaterial init(BlockRegistry blocksRegistry, ItemRegistry itemsRegistry) {
        ...
        RecipeDataProvider.defer(this::provideRecipes);

        initFlammable(FlammableBlockRegistry.getDefaultInstance());
        return this;
    }

    private void provideRecipes() {
        initDefaultRecipes();
        getRecipeEntries().forEach(entry -> entry.init(this));
    }
    ...
    /**
     * Allows to add blocks into {@link FlammableBlockRegistry} for this {@link ComplexMaterial}, not required.
     */
