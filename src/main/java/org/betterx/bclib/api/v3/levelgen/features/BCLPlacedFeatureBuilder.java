...
public class BCLPlacedFeatureBuilder<F extends Feature<FC>, FC extends FeatureConfiguration> extends CommonPlacedFeatureBuilder<F, FC, BCLPlacedFeatureBuilder<F, FC>> {
    ...
    public static void registerUnbound(BootstapContext<PlacedFeature> bootstrapContext) {
        org.betterx.bclib.BCLib.LOGGER.info("[datagen] registerUnbound PlacedFeatures: {}", UNBOUND_FEATURES.size());
        UNBOUND_FEATURES.forEach(u -> {
            org.betterx.bclib.BCLib.LOGGER.info(
                    "[datagen]   placed_feature {}",
                    u.getPlacedFeature()
                            .unwrapKey()
                            .map(k -> ((ResourceKey<?>) k).location())
                            .orElse(null)
            );
            u.register(bootstrapContext);
        });
