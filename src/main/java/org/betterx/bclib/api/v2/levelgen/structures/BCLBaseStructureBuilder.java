...
abstract class BCLBaseStructureBuilder<S extends Structure, T extends BCLBaseStructureBuilder<S, T>> {
    ...
    static void registerUnbound(BootstapContext<Structure> context) {
        org.betterx.bclib.BCLib.LOGGER.info("[datagen] registerUnbound Structures: {}", UNBOUND_STRUCTURES.size());
        UNBOUND_STRUCTURES.forEach(s -> {
            org.betterx.bclib.BCLib.LOGGER.info("[datagen]   structure {}", s.structureKey.location());
            var typeKey = net.minecraft.core.registries.BuiltInRegistries.STRUCTURE_TYPE.getKey(s.structureType);
            if (typeKey == null) {
                org.betterx.bclib.BCLib.LOGGER.error(
                        "[datagen]   !! StructureType has no registry key: {}",
                        s.structureType
                );
            }
            s.register(context);
    ...
    static void registerUnboundSets(BootstapContext<StructureSet> context) {
        org.betterx.bclib.BCLib.LOGGER.info("[datagen] registerUnbound StructureSets: {}", UNBOUND_STRUCTURE_SETS.size());
        UNBOUND_STRUCTURE_SETS.forEach(s -> {
            org.betterx.bclib.BCLib.LOGGER.info("[datagen]   structure_set {}", s.structureSetKey.location());
            s.registerSet(context);
        });
