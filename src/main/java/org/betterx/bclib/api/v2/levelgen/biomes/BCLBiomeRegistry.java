...
public class BCLBiomeRegistry {
    ...
    public static final Registry<Codec<? extends BCLBiome>> BIOME_CODECS =
            new MappedRegistry<>(BCL_BIOME_CODEC_REGISTRY, Lifecycle.stable());
    ...
    private static boolean CODECS_REGISTERED = false;
    ...
    public static ResourceKey<BCLBiome> registerForDatagen(BCLBiome biome) {
        ...
        BCLib.LOGGER.info("[datagen] register BCLBiome {}", biome.getBCLBiomeKey().location());
        Registry.register(
                BUILTIN_BCL_BIOMES,
...
//        }
    ...
    private static void onBiomeLoad(ResourceLocation id, BCLBiome biome) {
        if (!"minecraft".equals(id.getNamespace())) {
            if (biome.getIntendedType().is(BiomeAPI.BiomeType.BCL_NETHER)) {
                for (var params : biome.parameterPoints) {
                    NetherBiomesHelper.addNetherBiome(biome.getBiomeKey(), params);
                }
            } else if (biome.getIntendedType().is(BiomeAPI.BiomeType.BCL_END_CENTER)) {
                TheEndBiomesHelper.addMainIslandBiome(biome.getBiomeKey(), 1.0);
            } else if (biome.getIntendedType().is(BiomeAPI.BiomeType.BCL_END_BARRENS)) {
                TheEndBiomesHelper.addBarrensBiome(biome.getParentBiome().getBiomeKey(), biome.getBiomeKey(), 1.0);
            } else if (biome.getIntendedType().is(BiomeAPI.BiomeType.BCL_END_LAND)) {
                TheEndBiomesHelper.addHighlandsBiome(biome.getBiomeKey(), 1.0);
                TheEndBiomesHelper.addMidlandsBiome(biome.getBiomeKey(), biome.getBiomeKey(), 1.0);
            } else if (biome.getIntendedType().is(BiomeAPI.BiomeType.BCL_END_VOID)) {
                TheEndBiomesHelper.addSmallIslandsBiome(biome.getBiomeKey(), 1.0);
            } else {
