...
import org.betterx.bclib.mixin.common.PoiTypesAccessor;
...
public class PoiManager {
    public static BCLPoiType register(
        ...
        PoiType type;
        try {
            type = PoiTypesAccessor.bclib_register(
            ...
            if (BCLib.isDatagen()) {
                type = new PoiType(matchingStates, maxTickets, validRanges);
            } else {
                throw err;
            }
        }
    ...
    private static Map<BlockState, PoiType> ORIGINAL_BLOCK_STATES = null;
    ...
    public static void updateStates() {
        ...
        if (ORIGINAL_BLOCK_STATES == null) {
            ...
            ORIGINAL_BLOCK_STATES = new HashMap<>(PoiTypesAccessor.bclib_getTypeByState());
        } else {
            //restore unaltered state
            Map<BlockState, PoiType> typeByState = PoiTypesAccessor.bclib_getTypeByState();
            typeByState.clear();
            typeByState.putAll(ORIGINAL_BLOCK_STATES);
        }
                ...
                if (tag != null) {
                    ...
                    for (var block : registry.getTagOrEmpty(tag)) {
                        for (var state : block.value().getStateDefinition().getPossibleStates()) {
                            typeByState.put(state, type.value());
                        }
                    }
                }
            }
