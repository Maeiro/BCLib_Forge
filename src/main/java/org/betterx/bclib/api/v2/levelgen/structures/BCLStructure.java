...
@Mod.EventBusSubscriber(bus = Mod.EventBusSubscriber.Bus.MOD, modid = BCLib.MOD_ID)
public abstract class BCLStructure<S extends Structure> {
    ...
    private static <S extends Structure> StructureType<S> registerStructureType(
        ...
        StructureType<S> existing = (StructureType<S>) STRUCTURE_TYPES.get(id);
        if (existing != null) {
            return existing;
        }
        StructureType<S> type = () -> codec;
        STRUCTURE_TYPES.put(id, type);
        return type;
    }
    ...
    public static void registerForDatagen() {
        ...
        BCLib.LOGGER.info("[datagen] registering {} STRUCTURE_TYPES: {}", STRUCTURE_TYPES.size(), STRUCTURE_TYPES.keySet());
        STRUCTURE_TYPES.forEach((id, type) -> {
    ...
    public static void onRegister(RegisterEvent event) {
        if (event.getRegistryKey().equals(Registries.STRUCTURE_TYPE)) {
            event.register(Registries.STRUCTURE_TYPE, helper -> STRUCTURE_TYPES.forEach((id, type) -> {
                if (!BuiltInRegistries.STRUCTURE_TYPE.containsKey(id)) {
