...
public class BCLBiomeBuilder {
    ...
    public static int calculateSkyColor(float temperature) {
        float value = Mth.clamp(temperature / 3.0F, -1.0F, 1.0F);
    ...
    private final List<Pair<GenerationStep.Carving, ResourceKey<ConfiguredWorldCarver<?>>>> carvers = new ArrayList<>(
            1);
    ...
    public BCLBiomeBuilder carver(
        ...
        carvers.add(new Pair<>(step, carverKey));
        return this;
    }
    ...
    public <T extends BCLBiome> BCLBiomeContainer<T> build(BiomeSupplier<T> biomeConstructor) {
        ...
        final UnboundBCLBiome<T> unbound = new UnboundBCLBiome<>(
                ...
                ctx -> {
                    ...
                    carvers.forEach(entry -> genBuilder.addCarver(entry.first, entry.second));
