...
import org.betterx.worlds.together.util.ModEnvironment;
...
public class ModListScreen extends BCLibLayoutScreen {
    ...
    public static List<String> serverMissing(HelloClient.IServerModMap serverInfo) {
        return ModUtil.getMods().entrySet()
                      .stream()
                      .filter(entry -> entry.getValue().getEnvironment() != ModEnvironment.CLIENT)
    ...
    public static void addModDesc(
        ...
        mods.forEach(mod -> {
            ...
            if (serverInfo != null) {
                ...
                if (data != null) {
                    ...
                    if (!modVer.equals(mod.getVersion())) {
                        if (mod.getEnvironment() == ModEnvironment.CLIENT)
                ...
                } else if (mod.getEnvironment() == ModEnvironment.CLIENT) {
            ...
            String stateString = mod.getVersion();
            ...
            } else if (mod.getEnvironment() == ModEnvironment.SERVER) {
                stateString += ", server-only";
            }
            items.add(new Triple<>(mod.getName(), state, stateString));
        });
